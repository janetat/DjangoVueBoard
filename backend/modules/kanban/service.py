...
def is_board_existed(board_id):
    return Board.is_existed(board_id)
...
def get_board_data_by_board_id(board_id):
    """
    data included: board and pipeline and card.
    :param int board_id:
    :return:
    :rtype: dict
    """
...
def add_pipe_line(board_id, pipe_line_name):
    board = Board.get_by_id(board_id)
    current_count = PipeLine.get_current_pipe_line_count_by_board(board) - 1
...
def add_card(pipe_line_id, card_title):
    pipe_line = PipeLine.get_by_id(pipe_line_id)
    current_count = Card.get_current_card_count_by_pipe_line(pipe_line) - 1
    return Card.create(
        title=card_title,
        content=None,
        pipe_line=pipe_line,
